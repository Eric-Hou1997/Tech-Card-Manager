...
func TestCompareCardVersion(t *testing.T) {
	...
	if _, err := compareCardVersion("latest", "v4.0.2"); err == nil {
...
func TestSelectCardUpdateAssetRequiresCanonicalName(t *testing.T) {
	release := cardGitHubRelease{
		...
		Assets: []cardGitHubReleaseAsset{
			{Name: "TCM-v4.0.2-MacOS-AArch64-APP.zip", BrowserDownloadURL: "wrong-platform"},
			{Name: "TCM-v4.0.2-Windows-x64-EXE.zip", BrowserDownloadURL: "package"},
		},
	}
	...
	release.Assets = []cardGitHubReleaseAsset{{Name: "Tech-Card-Manager-Windows-x64-v4.0.2.zip"}}
